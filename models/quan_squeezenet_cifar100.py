...
class Fire(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, squzee_channel):
        ...
        self.squeeze = nn.Sequential(
            quan_Conv2d(in_channel, squzee_channel, 1),
            nn.BatchNorm2d(squzee_channel),
            nn.ReLU(inplace=True)
        )

        self.expand_1x1 = nn.Sequential(
            quan_Conv2d(squzee_channel, int(out_channel / 2), 1),
            nn.BatchNorm2d(int(out_channel / 2)),
            nn.ReLU(inplace=True)
        )

        self.expand_3x3 = nn.Sequential(
            quan_Conv2d(squzee_channel, int(out_channel / 2), 3, padding=1),
            nn.BatchNorm2d(int(out_channel / 2)),
            nn.ReLU(inplace=True)
        )
...
class SqueezeNet(nn.Module):
    ...
    def __init__(self, num_classes=100):
        ...
        self.conv10 = quan_Conv2d(512, num_classes, 1)
...
def squeezenet_quan(num_classes=100):
