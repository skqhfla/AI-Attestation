...
# AI-Attestation/models/quan_wyze.py
# Adapted for Python 3.10 and Linux environment.

SCRIPT_DIR = Path(__file__).resolve().parent
MODEL_BLOB_PATH = SCRIPT_DIR / "blobs" / "model_blob"
WEIGHT_BLOB_PATH = SCRIPT_DIR / "blobs" / "weight_blob"
...
class WyzeClassifySTE(nn.Module):
    ...
    def _parse_and_build(self):
        if not MODEL_BLOB_PATH.exists() or not WEIGHT_BLOB_PATH.exists():
             raise FileNotFoundError(f"Blobs not found at {MODEL_BLOB_PATH} or {WEIGHT_BLOB_PATH}")
...
def wyze_resnet20_quan(pretrained=True):
    # This matches the signature expected by AI-Attestation/__init__.py
