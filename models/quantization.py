...
class quan_Conv2d(nn.Conv2d):
    ...
    def forward(self, input):
        if self.inf_with_weight:
            ...
            self.__reset_stepsize__()
            device = input.device
            weight_quan = quantize(self.weight.to(device), self.step_size.to(device),
                                   self.half_lvls) * self.step_size.to(device)
            bias = self.bias.to(device) if self.bias is not None else None
            return F.conv2d(input, weight_quan, bias, self.stride,
    ...
    def forward(self, input):
        if self.inf_with_weight:
            ...
            weight_quan = quantize(self.weight.to(device), self.step_size.to(device),
                                   self.half_lvls) * self.step_size.to(device)
            bias = self.bias.to(device) if self.bias is not None else None
            return F.linear(input, weight_quan, bias)
