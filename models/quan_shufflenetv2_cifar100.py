    ...
    def __init__(self, in_channels, out_channels, stride):
        ...
        if stride != 1 or in_channels != out_channels:
            self.residual = nn.Sequential(
                quan_Conv2d(in_channels, in_channels, 1),
                nn.BatchNorm2d(in_channels),
                nn.ReLU(inplace=True),
                quan_Conv2d(in_channels, in_channels, 3, stride=stride, padding=1, groups=in_channels),
                nn.BatchNorm2d(in_channels),
                quan_Conv2d(in_channels, int(out_channels / 2), 1),
...
class ShuffleNetV2(nn.Module):
    ...
    def __init__(self, ratio=1, num_classes=100):
        ...
        self.conv5 = nn.Sequential(
            quan_Conv2d(out_channels[2], out_channels[3], 1),
            nn.BatchNorm2d(out_channels[3]),
            nn.ReLU(inplace=True)
        )

        self.fc = quan_Linear(out_channels[3], num_classes)
...
def shufflenetv2_quan(num_classes=100):
