import random
import torch
from .quantization import quan_Conv2d, quan_Linear, quantize
import operator
    ...
    def progressive_bit_search(self, model, data, target):
        ...
        while self.loss_max <= self.loss.item():
            ...
            for name, module in model.named_modules():
                if isinstance(module, (quan_Conv2d, quan_Linear)): 
            ...
            print(f"max loss module {max_loss_module}")
            ...
            if name == max_loss_module:
                ...
                for i in range(attack_weight_idx.size()[0]):
                    ...
                    tmp_list = [module_idx, # module index in the net
                                ...
                                idx_tuple, # attacked weight index in weight tensor
    ...
    def random_flip_one_bit(self, model):
        ...
        for name, m in model.named_modules():
            if name == chosen_module:
                ...
                bin_w = int2bin(flatten_weight[chosen_idx], m.N_bits).to(torch.int16)
